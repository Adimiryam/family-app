      ...
      <div style={{ background: 'white', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, margin: '0 auto', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '16px 20px 12px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{LOCALITIES.length} יישובים מפיקוד העורף</p>
          </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '8px 16px' }}>
          {displayList.map(person => {
            ...
            const lastUpdated  = draft[person.id]?.updatedAt
            const cityData     = currentCity ? cityAlertData[currentCity] : null
            const cfg          = cityData ? alertLevelConfig[cityData.level] : alertLevelConfig['low']
            const memberSearch = search[person.id] || ''
            const filtered     = memberSearch ? LOCALITIES_SORTED.filter(l => l.name.includes(memberSearch)) : LOCALITIES_SORTED

            return (
              <div key={person.id} style={{ padding: '10px 0', borderBottom: '1px solid #f1f5f9' }}>
                {/* שם + מיקום נוכחי */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  <span style={{ fontSize: 20, flexShrink: 0 }}>{person.emoji}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 14, fontWeight: 800, color: '#1e293b' }}>{person.name}</span>
                      {person.military && <span style={{ fontSize: 11 }}>🪖</span>}
                      <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: currentCity ? cfg.bg : '#fef3c7', color: currentCity ? cfg.color : '#92400e', fontWeight: 700 }}>
                        📍 {currentCity || 'לא הוגדר'}
                      </span>
                    </div>
                    {lastUpdated && (
                ...
                {/* בסיס כלשהו — בחירה מהירה בראש */}
                <button
                  onClick={() => handleChange(person, SPECIAL_BASE.name)}
                  style={{
                    ...
                    border: currentCity === SPECIAL_BASE.name ? 'none' : '1.5px solid #86efac',
                    width: '100%',
                  }}
                >
                  <span>🪖</span>
                  <span>בסיס כלשהו</span>
                  {currentCity === SPECIAL_BASE.name && <span style={{ marginRight: 'auto' }}>✓</span>}
                </button>

                {/* חיפוש + תפריט */}
                <input
                  value={memberSearch}
                  onChange={e => setSearch(s => ({ ...s, [person.id]: e.target.value }))}
                  placeholder={`חפש/י יישוב...`}
                  style={{ width: '100%', padding: '7px 10px', borderRadius: '8px 8px 0 0', border: '1.5px solid #e2e8f0', borderBottom: memberSearch ? '1.5px solid #3b82f6' : '1px solid #e2e8f0', fontSize: 13, background: '#f8fafc', direction: 'rtl' }}
                />
                <select
                  value={currentCity || ''}
                  onChange={e => e.target.value ? handleChange(person, e.target.value) : null}
                  size={memberSearch ? Math.min(filtered.length + 1, 5) : 1}
                  style={{ width: '100%', padding: '7px 10px', borderRadius: '0 0 8px 8px', border: '1.5px solid #e2e8f0', borderTop: memberSearch ? 'none' : undefined, fontSize: 13, background: 'white', direction: 'rtl' }}
                >
                  {!currentCity && <option value="">— בחר/י ישוב —</option>}
                  {filtered.map(l => <option key={l.name} value={l.name}>{l.name}</option>)}
                </select>
              </div>
            )
          })}
